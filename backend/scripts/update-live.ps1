...
try {
    ...
    if (Test-Path $hostsPath) {
        $hostOverride = Select-String -Path $hostsPath -Pattern "(?im)^\s*127\.0\.0\.1\s+devbits\.app(\s|$)" -ErrorAction SilentlyContinue
        if ($hostOverride) {
            Write-Host "WARNING: hosts file maps devbits.app to 127.0.0.1. This can cause domain/API connection failures and slow first loads." -ForegroundColor Yellow
